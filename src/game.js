...
import { TumbleTempleLevel } from './levels/tumbleTemple.js';
import { CrownClimbLevel } from './levels/crownClimb.js';

const LEVEL_CLASSES = [DoorDashLevel, SkylineCircuitLevel, SpinCycleLevel, FruitFrenzyLevel, HoverboardRunLevel, TumbleTempleLevel, CrownClimbLevel];
const LEVEL_BY_NAME = {
  ...
  'TUMBLE TEMPLE': TumbleTempleLevel,
  'CROWN CLIMB': CrownClimbLevel,
};
...
export class Game {
  ...
  _pickLevels() {
    // Four-round show: pick a random sequence each match, with Hex Havoc kept
    // as the finale so a full win awards a crown.
    const pool = [DoorDashLevel, SkylineCircuitLevel, SpinCycleLevel, FruitFrenzyLevel, HoverboardRunLevel, TumbleTempleLevel, CrownClimbLevel];
    const rng = mulberry32(makeSeed());
    const shuffled = pool.slice().sort(() => rng() - 0.5);
    return [...shuffled.slice(0, 3), HexHavocLevel];
  }
  ...
  _loadLevel(LevelClass, seed) {
    ...
    this.level.build();
    if (this.level.addSpawnSlime) this.level.addSpawnSlime();
  }
  ...
  _hostCheckRoundProgress(dt) {
    ...
    this.characters.forEach((ch, id) => {
      ...
      if (this.level.tryTouchTileUnder) {
        const tileIndex = this.level.tryTouchTileUnder(ch);
  ...
  update() {
    ...
    if (local) {
      ...
      if (localCanControl) {
        local.inSlimeZone = !!(this.level && this.level.isSlimeAt && this.level.isSlimeAt(local.body.position));
    ...
    if (this.isHost) {
      ...
      this.remoteInputs.forEach((inputData, peerId) => {
        ...
        ch.inSlimeZone = !!(this.level && this.level.isSlimeAt && this.level.isSlimeAt(ch.body.position));
    ...
    if (this.level && this.level.resolveCharacterCollision) {
      this.characters.forEach((ch, id) => {
        if (!this.activeIds || !this.activeIds.has(id) || ch.eliminated || ch.qualified) return;
        this.level.resolveCharacterCollision(ch);
      });
    }
