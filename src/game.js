...
export class Game {
  ...
  addRemotePlayer(peerId, name, color) {
  ...
  update() {
    ...
    if (local) {
      ...
      if (!this.isHost && this.netAccum >= INPUT_SEND_RATE) {
        ...
        this.network.sendInput({
          x: moveInput.x, z: moveInput.z, jump: moveInput.jump, dive: moveInput.dive,
    ...
    if (this.isHost) {
      // apply remote inputs to their characters before physics step
      this.remoteInputs.forEach((inputData, peerId) => {
        const ch = this.characters.get(peerId);
        if (ch) ch.applyInput(inputData, inputData.yaw || 0, dt);
      });
    }
