...
const MOVE_ACCEL = 240;
const AIR_CONTROL = 0.42;
const MAX_SPEED = 18.5;
...
const GROUND_RAY_EXTRA = 0.22;
const GROUND_SNAP_TOLERANCE = 0.34;
...
export class Character {
  ...
  _groundProbe() {
    ...
    this.world.raycastAll(from, to, { skipBackfaces: true }, (result) => {
      // Cannon rays can hit the player's own compound shapes first. That was the
      // real cause of the infinite jump/fly-hack: the controller thought it was
      // grounded while airborne because it was raycasting into itself.
    ...
    if (closeEnough) {
      ...
      if (Math.abs(this.body.velocity.y) < 1.5) this.jumpConsumed = false;
  ...
  applyInput(input, camYaw, dt) {
    ...
    if (this.state === STATE.TUMBLE) {
      ...
      if (hasInput) {
        this.body.velocity.x += wx * MOVE_ACCEL * 0.16 * dt;
        this.body.velocity.z += wz * MOVE_ACCEL * 0.16 * dt;
      }
    ...
    if (this.state === STATE.RECOVER) {
      ...
      if (hasInput) {
        this.body.velocity.x += wx * MOVE_ACCEL * 0.34 * dt;
        this.body.velocity.z += wz * MOVE_ACCEL * 0.34 * dt;
      }
    ...
    if (hasInput && canGroundAct && !isDiving) {
      this.body.velocity.x = THREE.MathUtils.lerp(this.body.velocity.x, targetVX, Math.min(1, dt * 18));
      this.body.velocity.z = THREE.MathUtils.lerp(this.body.velocity.z, targetVZ, Math.min(1, dt * 18));
    }
