...
export function buildCharacterMesh(color, cosmetic = {}) {
  ...
  if (skin === 'Lightning') {
    ...
    const houseMat = toonMat(0x8b5a2b);
    const cabin = new THREE.Mesh(new THREE.BoxGeometry(0.86, 1.05, 0.52), houseMat);
    cabin.position.set(0, 0.78, 0.02);
    cabin.castShadow = true;
    group.add(cabin);
    const roof = new THREE.Mesh(new THREE.ConeGeometry(0.72, 0.36, 4), toonMat(0x4b2c5e));
    roof.position.set(0, 1.47, 0.02);
    roof.rotation.y = Math.PI / 4;
    roof.castShadow = true;
    group.add(roof);
    const moon = new THREE.Mesh(new THREE.TorusGeometry(0.085, 0.022, 8, 18, Math.PI * 1.55), toonMat(0xffd23f));
    moon.position.set(0, 1.02, 0.31);
    moon.rotation.set(0, 0, -0.55);
    group.add(moon);
    const handle = new THREE.Mesh(new THREE.SphereGeometry(0.035, 8, 6), toonMat(0xffd23f));
    handle.position.set(0.26, 0.78, 0.32);
    group.add(handle);
  } else if (skin === 'Crown Knight') {
    const helm = new THREE.Mesh(new THREE.SphereGeometry(0.53, 22, 14), toonMat(0xdedede, { transparent: true, opacity: 0.72 }));
    helm.scale.set(1.05, 0.82, 0.72);
    helm.position.set(0, 1.05, 0.05);
    helm.castShadow = true;
    group.add(helm);
    for (let i = -1; i <= 1; i++) {
      const spike = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.22, 4), toonMat(0xffd23f));
      spike.position.set(i * 0.18, 1.52, 0.22);
      spike.castShadow = true;
      group.add(spike);
    }
  } else if (skin === 'Soda Can') {
    const label = new THREE.Mesh(new THREE.CylinderGeometry(0.50, 0.50, 1.26, 24, 1, true), toonMat(0xff5fa2, { transparent: true, opacity: 0.88 }));
    label.position.set(0, 0.72, 0);
    label.castShadow = true;
    group.add(label);
    const tab = new THREE.Mesh(new THREE.TorusGeometry(0.12, 0.025, 8, 20), toonMat(0xffffff));
    tab.position.set(0, 1.45, 0.08);
    tab.rotation.x = Math.PI / 2;
    group.add(tab);
  } else if (skin === 'Traffic Cone') {
    const cone = new THREE.Mesh(new THREE.ConeGeometry(0.58, 1.28, 24, 1, true), toonMat(0xff8a00, { transparent: true, opacity: 0.76 }));
    cone.position.set(0, 0.86, 0.02);
    cone.castShadow = true;
    group.add(cone);
    const stripe1 = new THREE.Mesh(new THREE.TorusGeometry(0.30, 0.035, 8, 28), toonMat(0xffffff));
    stripe1.position.set(0, 0.74, 0.02); stripe1.rotation.x = Math.PI / 2; group.add(stripe1);
    const stripe2 = stripe1.clone(); stripe2.scale.set(0.66,0.66,0.66); stripe2.position.y = 1.05; group.add(stripe2);
...
export class Character {
  ...
  applyInput(input, camYaw, dt) {
    ...
    const supported = canGroundAct || nearFloor || this.spawnGraceTimer > 0;
    ...
    this._desiredGroundVelocity = hasInput && !isDiving ? { x: targetVX, z: targetVZ } : null;
  ...
  _enforceGroundMove(dt) {
    // Final anti-glue guard. This is intentionally blunt: while a player/bot is
    // giving movement input in RUN/RECOVER, the physics solver is not allowed to
    // leave them crawling at 5% speed on spawn platforms. Wall contacts still run
    // immediately after this and remove velocity into solid obstacles/doors.
    if (!this._desiredGroundVelocity) return;
    if (this.state !== STATE.RUN && this.state !== STATE.RECOVER) return;

    const tx = this._desiredGroundVelocity.x;
    const tz = this._desiredGroundVelocity.z;
    const desiredSpeed = Math.hypot(tx, tz);
    if (desiredSpeed < 0.01) return;

    // If we are pushing straight into a wall/solid door, do not override that.
    for (const n of this.contactNormals) {
      if (n.y < 0.25) {
        const intoDesiredWall = tx * n.x + tz * n.z;
        if (intoDesiredWall < -desiredSpeed * 0.35) return;
      }
    }
    ...
    const minAlong = desiredSpeed * 0.98;

    if (curAlong < minAlong || curSpeed < desiredSpeed * 0.55) {
