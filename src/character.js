...
export class Character {
  ...
  applyInput(input, camYaw, dt) {
    ...
    // Failsafe for thin/platform edge contacts: if Cannon is supporting the
    // body vertically, do not let one bad raycast frame downgrade the player to
    // weak air-control while they are clearly on the floor.
    const supported = canGroundAct || (this.airTime < 0.08 && this.body.velocity.y > -0.35 && this.body.velocity.y < 0.35);
    ...
    this._desiredGroundVelocity = hasInput && supported && !isDiving ? { x: targetVX, z: targetVZ } : null;
  ...
  _enforceGroundMove(dt) {
    // Last-resort arcade controller guard: Cannon contact solving can still eat
    // horizontal speed on crowded spawn pads or thin Hex tiles. When input asks
    // for movement and the feet are supported, restore the intended ground
    // velocity after the physics step so the floor cannot become glue.
    if (!this._desiredGroundVelocity) return;
    if (this.state !== STATE.RUN && this.state !== STATE.RECOVER) return;
    const supportedNow = this.grounded || this.coyoteTimer > 0 || this.spawnGraceTimer > 0;
    ...
    const minAlong = desiredSpeed * 0.72;
    if (curAlong < minAlong) {
      const nx = tx / desiredSpeed;
      const nz = tz / desiredSpeed;
      const add = minAlong - curAlong;
      this.body.velocity.x += nx * add;
      this.body.velocity.z += nz * add;
    }
  }
