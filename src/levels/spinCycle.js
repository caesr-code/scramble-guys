...
export class SpinCycleLevel extends BaseLevel {
  ...
  build() {
    ...
    spinnerZs.forEach((z, i) => {
      ...
      // Shared seeded RNG (see rng.js) - a spinning hazard's rotational phase
      // is gameplay-critical (it's a moving obstacle), so it must match
      // between host and clients exactly like the door pattern does.
      const rng = this.rng || Math.random;
      this.spinners.push({ mesh, body, angle: rng() * Math.PI * 2, speed, dir });
    });
