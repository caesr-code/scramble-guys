...
export class DoorDashLevel extends BaseLevel {
  ...
  resolveCharacterCollision(ch) {
    // Manual fake-door blocker. The slow-walk fix restores velocity after Cannon,
    // so fake Door Dash doors get a final explicit wall pass here. This only runs
    // near unbroken fake doors and never touches spawn/platform movement.
    const p = ch.body.position;
    for (const gate of this.gateWalls) {
      if (Math.abs(p.z - gate.z) > 2.2) continue;
      for (const door of gate.doors) {
        if (door.broken || door.isReal) continue;
        const halfW = door.doorWidth * 0.5 + 0.38;
        const halfD = 1.12;
        if (Math.abs(p.x - door.x) < halfW && Math.abs(p.z - door.z) < halfD && p.y < 4.2) {
          const fromFront = p.z > door.z;
          p.z = door.z + (fromFront ? halfD : -halfD);
          if (fromFront && ch.body.velocity.z < 0) ch.body.velocity.z = Math.max(0.25, Math.abs(ch.body.velocity.z) * 0.08);
          if (!fromFront && ch.body.velocity.z > 0) ch.body.velocity.z = Math.min(-0.25, -Math.abs(ch.body.velocity.z) * 0.08);
          const sidePush = Math.sign(p.x - door.x) || 1;
          if (Math.abs(p.x - door.x) > door.doorWidth * 0.46) p.x = door.x + sidePush * halfW;
          ch.body.velocity.x *= 0.35;
          if (ch.state === 'tumble') ch.body.velocity.y = Math.max(ch.body.velocity.y, 2.0);
          return;
        }
      }
    }
