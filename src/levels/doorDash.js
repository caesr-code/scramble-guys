...
export class DoorDashLevel extends BaseLevel {
  ...
  build() {
    ...
    // Start pad is VISUAL ONLY. The main course floor already exists here.
    // A second static physics floor on top of it caused the spawn-platform bug:
    // movement became tiny, jumping failed, and ragdoll recovery got stuck.
    const startPadGeo = new THREE.BoxGeometry(trackWidth, 0.08, 10);
    const startPadMat = new THREE.MeshToonMaterial({ color: 0xFFD23F });
    const startPad = new THREE.Mesh(startPadGeo, startPadMat);
    startPad.position.set(0, 0.04, 10);
    ...
    gateZs.forEach((z, gi) => {
      ...
      for (let i = 0; i < doorsPerGate; i++) {
        ...
        const cap = b.box(doorWidth - 0.1, 0.32, 0.62, new THREE.Vector3(x, 3.75, z), { color: 0x251B45 });
        const leftPost = b.box(0.12, 3.8, 0.62, new THREE.Vector3(x - doorWidth / 2 + 0.08, 1.9, z), { color: 0xFFFFFF });
        const rightPost = b.box(0.12, 3.8, 0.62, new THREE.Vector3(x + doorWidth / 2 - 0.08, 1.9, z), { color: 0xFFFFFF });
        gateGroup.doors.push({ mesh, body, isReal, broken: false, stripe, cap: cap.mesh, leftPost: leftPost.mesh, rightPost: rightPost.mesh, x, z });
      }
  ...
  breakDoor(door) {
    ...
    if (door.cap) door.cap.visible = false;
    if (door.leftPost) door.leftPost.visible = false;
    if (door.rightPost) door.rightPost.visible = false;
  }
