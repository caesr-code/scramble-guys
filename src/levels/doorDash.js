...
export class DoorDashLevel extends BaseLevel {
  ...
  build() {
    ...
    gateZs.forEach((z, gi) => {
      ...
      // ensure at least 2 real doors - MUST use the shared seeded RNG (this.rng,
      // set by game.js before build() runs), not Math.random(). Every client
      // builds this level independently; Math.random() would make each
      // machine pick a different real/fake pattern, so a door a client
      // considers permanently solid could vanish the moment the host (using
      // its own different pattern) decides that same slot was "real" and
      // broadcasts that it broke.
      const rng = this.rng || Math.random;
      while (realDoors.size < 2) realDoors.add(Math.floor(rng() * doorsPerGate));
      ...
      for (let i = 0; i < doorsPerGate; i++) {
        ...
        const { mesh, body } = b.box(doorWidth - 0.08, 3.85, 2.05, new THREE.Vector3(x, 1.92, z), {
        ...
        for (const ex of [-0.42, 0.42]) {
          ...
          eye.position.set(x + ex, 2.62, z + 0.78);
          ...
          pupil.position.set(x + ex, 2.59, z + 0.92);
          this.scene.add(eye, pupil);
          eyes.push(eye, pupil);
        }

        const cap = b.box(doorWidth - 0.1, 0.32, 2.05, new THREE.Vector3(x, 3.9, z), { color: 0x251B45 });
        const leftPost = b.box(0.12, 3.8, 2.05, new THREE.Vector3(x - doorWidth / 2 + 0.08, 1.95, z), { color: 0xFFFFFF });
        const rightPost = b.box(0.12, 3.8, 2.05, new THREE.Vector3(x + doorWidth / 2 - 0.08, 1.95, z), { color: 0xFFFFFF });
  ...
  tryBreakDoorsNear(position) {
    for (let gi = 0; gi < this.gateWalls.length; gi++) {
      ...
      for (let di = 0; di < gate.doors.length; di++) {
        ...
        if (Math.abs(dz) < 2.05 && Math.abs(dx) < door.doorWidth * 0.46) {
          if (door.isReal) {
            this.breakDoor(door);
  ...
  // Extra Door Dash safety net: Cannon can occasionally miss a fast compound
  // bean against a static door in the browser build, especially in multiplayer
  // prediction. Fake doors are meant to be absolutely solid, so clamp the bean
  // back to the side it came from and bounce it instead of letting it noclip.
  enforceSolidDoorCollisions(character) {
    if (!character || !character.body) return;
    const p = character.body.position;
    const v = character.body.velocity;
    const beanRadius = 0.62;

    for (const gate of this.gateWalls) {
      for (const door of gate.doors) {
        if (door.broken) continue;
        const halfW = door.doorWidth * 0.5 + beanRadius * 0.55;
        const halfD = 1.12 + beanRadius;
        const dx = p.x - door.x;
        const dz = p.z - door.z;
        if (Math.abs(dx) > halfW || Math.abs(dz) > halfD || p.y < 0.25 || p.y > 4.25) continue;
        ...
        // Course direction is toward negative Z. Push to whichever face is
        // closest, with a strong preference for stopping forward penetration.
        const fromFront = dz >= 0 || v.z < 0;
        p.z = door.z + (fromFront ? halfD : -halfD);
        if (fromFront) v.z = Math.max(v.z, 3.0);
        else v.z = Math.min(v.z, -2.0);
        v.x *= 0.35;
        if (v.y < 0) v.y *= 0.25;
        return;
      }
  ...
  applySpecialForces(character) {
    // Run before the physics step to pre-break real doors and pre-clamp fake
    // doors. Host round-progress still broadcasts door breaks for multiplayer;
    // this just makes the local/player physics feel solid immediately.
    this.enforceSolidDoorCollisions(character);
  }
