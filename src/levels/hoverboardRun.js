...
export class HoverboardRunLevel extends BaseLevel {
  ...
  build() {
    ...
    this.platform = b.box(13, 0.75, 13, new THREE.Vector3(0, -0.05, -8), { color: 0x3DDCFF, grid: true });
    ...
    this.spawnPoints = [];
    for (let row = 0; row < 4; row++) for (let col = 0; col < 5; col++) this.spawnPoints.push(new THREE.Vector3(-5.6 + col * 2.8, 1.5, 15 + row * 1.8));
  }

  update(dt, t) {
    this.elapsed += dt;
    const boardZ = Math.max(this.finishZ + 5, -8 - this.elapsed * 3.0);
    this.platform.body.position.set(0, -0.05, boardZ);
    this.platform.body.velocity.set(0, 0, -3.0);
  ...
  isOutOfBounds(pos) {
    ...
    return pos.z > this.platform.body.position.z + 15 && pos.z < -12;
  }
