...
export class HexHavocLevel extends BaseLevel {
  ...
  build() {
    ...
    // soft visual lower tiers under the real collision layer, giving the finale
    // more depth without changing the Stage 4 survival rules.
    for (let tier = 1; tier <= 2; tier++) {
      const ring = b.cylinder(12 - tier * 2, 12 - tier * 2, 0.32, new THREE.Vector3(0, -tier * 2.2, 0), { color: tier === 1 ? 0x8C6FFF : 0xFF5FA2, radialSegments: 6 });
      ring.mesh.material.transparent = true;
      ring.mesh.material.opacity = 0.42;
      ring.body.collisionFilterMask = 0;
    }
