...
export class HexHavocLevel extends BaseLevel {
  ...
  _tileAtPosition(pos, velocity = null) {
    // Only count a tile as touched when the bean is actually standing on this
    // layer. The previous generous height window could mark lower layers while
    // the player was still falling, causing a chain reaction where every layer
    // vanished before the bean had any chance to move.
    let best = null;
    let bestD = Infinity;
    const maxDownSpeedForTouch = 7.5;
    if (velocity && velocity.y < -maxDownSpeedForTouch) return null;

    for (let i = 0; i < this.tiles.length; i++) {
      const tile = this.tiles[i];
      if (tile.dropped || tile.touched) continue;

      // Body-centre standing height for the compound bean is roughly tile top
      // plus the lower sphere/half-height. Keep this band tight so merely
      // passing through the air above/below a layer does not pre-delete it.
      const standingY = tile.baseY + 1.25;
      if (Math.abs(pos.y - standingY) > 0.62) continue;

      const dx = pos.x - tile.x;
      const dz = pos.z - tile.z;
      const d = Math.hypot(dx, dz);
      if (d < 1.42 && d < bestD) { best = { tile, index: i }; bestD = d; }
    }
    return best;
  }

  tryTouchTileUnder(pos, velocity = null) {
    const hit = this._tileAtPosition(pos, velocity);
    if (!hit) return null;
