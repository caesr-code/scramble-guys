...
export class HexHavocLevel extends BaseLevel {
  ...
  _tileAtPosition(pos) {
    // Body position is the centre of the bean collider. A stable standing bean
    // sits about 0.89 above the floor, and the hex tile top is baseY + 0.38.
    // So real foot contact is around baseY + 1.27. The previous ±2.25 band
    // triggered tiles while the player was still falling past them, which is
    // what caused the instant all-layers fall-through.
    let best = null;
    let bestD = Infinity;
    const STANDING_CENTER_OFFSET = 1.27;
    for (let i = 0; i < this.tiles.length; i++) {
      const tile = this.tiles[i];
      if (tile.dropped || tile.touched) continue;
      const expectedY = tile.baseY + STANDING_CENTER_OFFSET;
      if (Math.abs(pos.y - expectedY) > 0.62) continue;
      const dx = pos.x - tile.x;
      const dz = pos.z - tile.z;
      const d = Math.hypot(dx, dz);
      // Slightly smaller than the visual radius so edge fly-bys do not delete
      // tiles from a lower layer before the bean has actually landed on them.
      if (d < 1.18 && d < bestD) { best = { tile, index: i }; bestD = d; }
    }
    return best;
  }

  tryTouchTileUnder(characterOrPos) {
    const pos = characterOrPos.body ? characterOrPos.body.position : characterOrPos;
    const vel = characterOrPos.body ? characterOrPos.body.velocity : null;
    // Only a supported/near-supported bean may consume a tile. Fast downward
    // falling through the air is handled by applySpecialForces() snapping to a
    // tile first, then this marks that landed tile to drop.
    if (vel && Math.abs(vel.y) > 1.1) return null;
  ...
  applySpecialForces(ch, dt, t) {
    if (!ch || !ch.body || ch.eliminated || ch.qualified) return;
    const body = ch.body;
    const FOOT_OFFSET = 0.89;
    const TILE_TOP_OFFSET = 0.38;
    // Manual one-way floor catch for Hex. Cannon can occasionally miss a thin
    // island when many bodies/tiles update together; this catch makes each tile
    // behave like a reliable platform without adding invisible walls.
    if (body.velocity.y > 1.2) return;
    let best = null;
    let bestY = -Infinity;
    for (const tile of this.tiles) {
      if (tile.dropped) continue;
      const topY = tile.baseY + TILE_TOP_OFFSET;
      const footY = body.position.y - FOOT_OFFSET;
      if (footY < topY - 0.18 || footY > topY + 0.72) continue;
      const dx = body.position.x - tile.x;
      const dz = body.position.z - tile.z;
      if (Math.hypot(dx, dz) > 1.18) continue;
      if (topY > bestY) { best = tile; bestY = topY; }
    }
    if (best) {
      body.position.y = bestY + FOOT_OFFSET;
      if (body.velocity.y < 0) body.velocity.y = 0;
      ch.grounded = true;
      ch.coyoteTimer = 0.16;
      ch.airTime = 0;
      if (ch.lastSafePos) ch.lastSafePos.copy(body.position);
    }
  }
  ...
  touchTile(index) {
    ...
    tile.touched = true;
    // Give the player time to actually react after landing. The old 0.42s
    // timer, combined with early mid-air touches, made whole layers vanish
    // before the bean could move.
    tile.touchTimer = 0.95;
