...
export class HexHavocLevel extends BaseLevel {
  ...
  build() {
    ...
    const addHex = (q, r, layer) => {
      ...
      const { mesh, body } = b.hexPrism(hexRadius * 0.97, 0.38, new THREE.Vector3(x, y, z), { color, mass: 0 });
  ...
  _tileAtPosition(pos) {
    ...
    for (let i = 0; i < this.tiles.length; i++) {
      ...
      if (Math.abs(pos.y - (tile.baseY + 1.05)) > 1.65) continue;
      const dx = pos.x - tile.x;
      const dz = pos.z - tile.z;
      const d = Math.hypot(dx, dz);
      if (d < 1.22 && d < bestD) { best = { tile, index: i }; bestD = d; }
    }
    return best;
  }
  ...
  dropRandomTiles(count) {
    // Only used as late-match cleanup pressure so stalemates cannot last
    // forever. Player contact is the primary way tiles disappear in Stage 6.
    ...
    for (let i = 0; i < count && candidates.length > 0; i++) {
      ...
      this.touchTile(this.tiles.indexOf(tile));
    }
  }
  ...
  update(dt, t) {
    ...
    // Stalemate pressure only after the first section of the finale.
    if (this.elapsed > 28) {
      const intensity = Math.min(1, (this.elapsed - 28) / 90);
      const interval = this.dropInterval * (1 - intensity * 0.35);
      if (this.dropTimer > interval) {
        this.dropTimer = 0;
        this.dropRandomTiles(1 + Math.floor(intensity * 1.5));
      }
    }
