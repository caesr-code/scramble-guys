...
export class HexHavocLevel extends BaseLevel {
  ...
  _tileAtPosition(pos) {
    // Kept for bots / safe-target helpers. Gameplay touching uses
    // _supportTileForCharacter() below so falling through the air cannot delete
    // tiles from every layer.
    let best = null;
    let bestD = Infinity;
  ...
  _supportTileForCharacter(ch) {
    ...
    for (let i = 0; i < this.tiles.length; i++) {
      ...
      // Only catch the nearest layer the feet are actually crossing/standing on.
      // This prevents a falling bean from consuming the blue/yellow layers below.
      if (footY < topY - 0.24 || footY > topY + 0.92) continue;
      const dx = body.position.x - tile.x;
      const dz = body.position.z - tile.z;
      if (Math.hypot(dx, dz) > 1.22) continue;
      if (topY > bestY) best = { tile, index: i, topY };
    }
    return best;
  }
  ...
  applySpecialForces(ch, dt, t) {
    ...
    const hit = this._supportTileForCharacter(ch);
    if (!hit) return;
    const body = ch.body;
    const FOOT_OFFSET = 0.89;
    body.position.y = Math.max(body.position.y, hit.topY + FOOT_OFFSET);
  ...
  tryTouchTileUnder(characterOrPos) {
    // Host only calls this. Require an actual supported character, not just a
    // position blob, so Hex tiles drop from feet-level contact instead of from
    // mid-air layer overlap.
    if (!characterOrPos || !characterOrPos.body) return null;
    const ch = characterOrPos;
    const vel = ch.body.velocity;
    if (vel.y < -0.75 || vel.y > 1.25) return null;
    const hit = this._supportTileForCharacter(ch);
    if (!hit || hit.tile.touched || hit.tile.dropped) return null;
